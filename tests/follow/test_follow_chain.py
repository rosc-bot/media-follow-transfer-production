...
@pytest.mark.asyncio
async def test_follow_scout_ingest_transfer_and_watchlist_writeback(tmp_path, monkeypatch):
    async def fake_metadata(self, payload, resource):
        return {
            "id": resource.tmdb_id,
            "media_type": "tv",
            "origin_country": ["CN"],
    ...
    monkeypatch.setattr(TransferQueueWorker, "_load_tmdb_metadata", fake_metadata)
