...
def test_edited_message_updates_row_and_reopens_outbox_only_for_changed_hash(tmp_path):
    path = tmp_path / "resource_messages.db"
    storage = ResourceStorage(str(path))
    ...
    with storage.get_conn() as db:
        unchanged = db.execute("SELECT status, payload FROM resource_outbox WHERE message_id=28157").fetchone()
    ...
    source = TelegramSourceMessage.model_validate(outbox_payload)
...
def test_resource_storage_adds_edit_columns_without_losing_legacy_rows(tmp_path):
    ...
    assert (row["text"], row["caption"], row["message_id"]) == ("旧标题", "", 28157)
    assert row["content_hash"]
    assert row["updated_at"] == "2026-09-23T00:00:00Z"
