...
async def record_candidate(
    ...
    *,
    ...
    canonical_key = canonical_episode_key(season, episode_key)
    episode_key = canonical_key or str(episode_key).strip()
...
async def mark_candidate_transferred(
    ...
    *,
    ...
    """Mark permanent success while preserving selection-attempt semantics.

    ``attempt_count`` is incremented when a candidate is selected/queued by
    ``mark_candidate_used``; a direct Canary may therefore legitimately have
    zero. Successful use always records ``last_used_at`` here.
    """
    if candidate is None:
        return
