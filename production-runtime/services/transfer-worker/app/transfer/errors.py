...
class TransferErrorCategory(StrEnum):
    ...
    FILE_SELECTION_REVIEW = 'FILE_SELECTION_REVIEW'
    TRANSFER_SCOPE_VIOLATION = 'TRANSFER_SCOPE_VIOLATION'
...
RETRY_POLICY: dict[TransferErrorCategory, TransferRetryPolicy] = {
    ...
    TransferErrorCategory.FILE_SELECTION_REVIEW: TransferRetryPolicy.TERMINAL,
    TransferErrorCategory.TRANSFER_SCOPE_VIOLATION: TransferRetryPolicy.TERMINAL,
...
class FileSelectionError(GuangyaTransferError):
    """The selected remote file scope is missing, ambiguous, or unsafe."""
    ...
    def __init__(
        ...
        message: str,
        *,
        category: TransferErrorCategory = TransferErrorCategory.FILE_SELECTION_REVIEW,
    ) -> None:
        self.code = code
...
class RenameUnverifiedError(GuangyaTransferError):
    """Restore is already verified; only the selected-file rename needs retrying."""

    def __init__(
        self,
        message: str = 'canonical filename rename did not pass readback verification',
        *,
...
class PromotionUnverifiedError(GuangyaTransferError):
    """Series-root move happened or was uncertain; retry only promotion readback."""

    def __init__(
        self,
        message: str = 'series promotion move did not pass complete readback verification',
        *,
        series_folder_id: str | None = None,
        remote_records: list[dict] | None = None,
        stage: str = 'MOVED',
    ) -> None:
        super().__init__(TransferErrorCategory.READBACK_UNVERIFIED, message)
        self.series_folder_id = series_folder_id
        self.remote_records = list(remote_records or [])
        self.stage = stage
