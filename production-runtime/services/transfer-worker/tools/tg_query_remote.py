...
def cmd_chats(conn):
    ...
    print(f"{'群聊名称':<36} {'ID':<20} {'消息数':<9} 最后活跃(北京)")
    print("-" * 92)
    for chat_id, title, n, last in rows:
        print(f"{(title or '?')[:34]:<36} {chat_id!s:<20} {n:<9d} {fmt(last)}")
...
def cmd_query(conn, args):
    ...
    if args.hours is not None:
        cutoff = datetime.now(UTC).timestamp() - args.hours * 3600
        where.append("date > ?")
    ...
    if not rows:
        print("⚠️ 未找到符合条件的消息")
        return
    print(f"共返回 {len(rows)} 条（默认最近优先；--since 按时间升序）：")
    priv = lambda cid: str(cid).removeprefix("-100")
    for rid, chat_id, mid, sid, sname, text, ts, ctitle in rows:
        t = fmt(ts)
        txt = (text or "[媒体消息/无文本]").replace("\n", " / ")
        url = f"https://t.me/c/{priv(chat_id)}/{mid}"
        print(f"[{t}] {sname or repr(sid)} (uid={sid}) @ {ctitle}\n     {txt[:200]}\n     {url}")
...
def cmd_stats(conn):
    ...
    print(f"总消息数: {n} | 群聊数: {chats} | 最后消息: {fmt(last)} (北京时间)")
