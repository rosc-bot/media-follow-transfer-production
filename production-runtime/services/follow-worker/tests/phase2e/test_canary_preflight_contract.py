...
@pytest.mark.asyncio
async def test_transient_share_api_failure_is_needs_review_not_resource_rejection(tmp_path):
    from tools.run_transfer_canary import run_preflight

    engine, sessions, task_id = await _canary_task(tmp_path)

    async def transient_share_error(**_kwargs):
        return {
            ...
            "failure_detail": "pagination safety limit",
        }

    report = await run_preflight(task_id, session_factory=sessions, remote_validator=transient_share_error)

    assert report["preflight_status"] == "NEEDS_REVIEW"
    assert report["checks"]["check_share_access"]["failure_code"] == "SHARE_API_ERROR"
    await engine.dispose()
