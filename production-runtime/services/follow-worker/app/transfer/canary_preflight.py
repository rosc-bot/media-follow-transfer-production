    ...
    *, provider: str, share_url: str, auth_ref: str, target_folder_id: str, episode_keys: list[str], season: int | None = None
    ...
    if not share["share_accessible"]:
        ...
        try:
            ...
            result["selection_result"] = selection.to_dict()
            ...
            elif selection.decision == "EPISODE_MISMATCH":
                result["failure_code"] = "EPISODE_MISMATCH"
                result["failure_detail"] = "no unique regular video file strictly matches target episode"
...
async def preflight_task(
    ...
    *,
    ...
    def fail(name: str, code: str, detail: str, *, review: bool = False) -> None:
        ...
        transient = code in {
            "NETWORK_TIMEOUT", "RATE_LIMITED", "SHARE_API_ERROR", "NETWORK_ERROR", "REMOTE_5XX",
        }
        if review or transient:
            if report["preflight_status"] == CANARY_SAFE:
    ...
    claimed_by_this_worker = (
        str(task.status) == str(TransferStatus.RUNNING)
        and bool(allow_running_locked_by)
        and str(task.locked_by or "") == str(allow_running_locked_by)
    )
    if task.status in EXECUTABLE_STATUSES or claimed_by_this_worker:
    ...
    if tmdb_id:
        ...
        resource_ids = {row.resource_id for row in related if row.resource_id is not None}
        related_resources = {
            row.id: row
        ...
        target_keys = {
            canonical
            for key in episode_keys
            if (canonical := canonical_episode_key(int(season or 1), key)) is not None
        }
        same = [
            row for row in related
            if row.id != task.id
            and task_matches_episode(
                row.payload,
                tmdb_id=int(tmdb_id),
                season=int(season or 1),
                episode_keys=target_keys,
                resource=related_resources.get(row.resource_id),
            )
        ]
