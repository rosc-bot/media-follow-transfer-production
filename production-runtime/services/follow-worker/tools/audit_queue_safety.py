...
COMPLETED_STATUSES = tuple(str(status) for status in SUCCESS_TERMINAL_STATUSES)


def normalize_episode_key(season: int | None, episode: object) -> str | None:
    """Canonicalize a legacy episode number or SxxExx token."""
    return canonical_episode_key(season, episode)


def season_from_episode_key(episode_key: str | None) -> int | None:
    canonical = canonical_episode_key(None, episode_key)
    if not canonical:
        return None
    return int(canonical.split("E", 1)[0][1:])
