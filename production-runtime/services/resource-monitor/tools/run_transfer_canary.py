...
async def _preflight(
    ...
    *,
    ...
    if tmdb_id and season:
        ...
        collected = set(map(str, (watchlist.collected_episodes if watchlist else []) or []))
        already = [ep for ep in map(str, episode_keys) if ep in collected]
        if already:
    ...
    # 8. no other SUCCESS task already completed the same episodes
    if tmdb_id:
        dup_tasks = (await db.execute(select(TransferQueueTask).where(
            TransferQueueTask.status == TransferStatus.COMPLETED,
        ))).scalars().all()
        dup_hits = []
        for t in dup_tasks:
            if not t.payload:
            ...
            if ({str(e) for e in t_eps} & {str(e) for e in episode_keys}
                    and (t.payload or {}).get('tmdb_id') == int(tmdb_id)):
                dup_hits.append(t.id)
        if dup_hits:
...
async def list_safe_candidates(
    ...
    *,
    ...
    async with session_factory() as db:
        ...
        for task in tasks:
            pre = await _preflight(db, task.id, remote_validator=remote_validator)
            resource = await db.get(Resource, task.resource_id) if task.resource_id else None
            report = {
                'task_id': task.id,
                'title': pre.get('title'),
...
async def run_execute(task_id: int, confirm_task_id: int) -> dict:
    ...
    result = await worker.execute_single_task(task_id, canary_override_pause=True)
