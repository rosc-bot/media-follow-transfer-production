...
@pytest.mark.asyncio
async def test_completed_ongoing_series_is_enqueued_for_one_safe_promotion(tmp_path):
    ...
    async with sessions() as db, db.begin():
        count = await CompletionPromotionService.enqueue_ready_promotions(db)
        assert count == 1
    ...
    async with sessions() as db:
        ...
        assert task.payload['series_folder_name'] == '迁移剧 (2025) {tmdbid-99}【完结】'
