...
class LayoutGuangyaAdapter(GuangyaAdapter):
    ...
    async def post(self, client, url, payload, headers):
        ...
        if url.endswith('/file/rename'):
            assert payload == {'fileId': 'legacy-series', 'newName': '旧剧集 (2025) {tmdbid-1}【完结】'}
            self.children['completed-root'][0]['name'] = payload['newName']
            return {'code': 0, 'data': {}}
...
async def test_completed_transfer_promotes_existing_ongoing_series_before_restoring_final_episode():
    ...
    outcome = await adapter.transfer({
        ...
        'auth_token': 'access-token',
        'expected_files': ['S01E02.mkv'],
        'series_folder_name': '旧剧集 (2025) {tmdbid-1}【完结】',
        'season_folder_name': 'S01',
